...
class GaussianDiffusion:
    def __init__(self,
        ...
        self.C_1 = np.sqrt(1.0 - self.alphas_cumprod_prev) / np.sqrt(1.0 - self.alphas_cumprod)
        self.C_2 = np.sqrt(self.alphas_cumprod_prev) - (np.sqrt(self.alphas_cumprod) * self.C_1)

    def n_frequency(self, t, n_max=50):
        signal_t = self.alphas_cumprod[t] / (1.0 - self.alphas_cumprod[t])
        frequency = np.floor(max(n_max*(1-np.exp(-signal_t)), 2))
    ...
    def p_sample_loop(self,
        ...
        """
        # aligned initialization proposed in the paper
        if measurement.shape[-1] != x_start.shape[-1]:  # means measurement is downsampled
            x_start = self.q_sample(
                torch.nn.functional.interpolate(measurement, scale_factor=x_start.shape[-1] // measurement.shape[-1]),
                self.num_timesteps - 1)
        else:
            x_start = self.q_sample(measurement, self.num_timesteps - 1)
        # -------------------------------------------------------
        ...
        for idx in pbar:
            ...
            out = self.p_mean_variance(x=img, t=time, model=model)

            # Give condition.
            noisy_measurement = self.q_sample(measurement, t=time)
            frequency = self.n_frequency(t=time)

            sqrt_alphas_cumprod_prev = extract_and_expand(np.sqrt(self.alphas_cumprod_prev), time, measurement)
            sqrt_one_minus_alphas_cumprod_prev = extract_and_expand(np.sqrt(1.0 - self.alphas_cumprod_prev), time, img)

            # TODO: how can we handle argument for different condition method?
            img, distance = measurement_cond_fn(x_t=out['mean'],
                                                ...
                                                sqrt_one_minus_alphas_cumprod_prev=sqrt_one_minus_alphas_cumprod_prev,
                                                **kwargs)
            img = img.detach_()
...
class DDPM(SpacedDiffusion):
    ...
    # def p_sample(self, model, x, t, measurement=None, variance=None):
    #     out = self.p_mean_variance(model, x, t)
    #     sample_mean = out['mean']
    #     # we need the mean, log_variance and pred_noise
    #     return {'sample_mean': sample_mean, 'pred_xstart': out['pred_xstart'],
    #             'log_variance': out['log_variance'], 'pred_noise': out['pred_noise']}
